...
const OUTPUT_HEADERS_BASE = [
  '業種','株探','四季','銘偵','全銘',
  '時価総額','PER','PBR','DY',
  'PO','RSI','週RSI','BB','RSI22R','買残日数','不安定率5','タイプ分類','連日',
  '特色','連結事業','決算発表予定日',
  '上場区分',
  '(96)AI基準判定',
  '四季報スコア',
...
const SOURCE_HEADER_MAP = {
  ...
  '週RSI':    '(128)週足RSI',
  'BB':       '(138)ボリンジャーバンドのσ値',
  'RSI22R':   '(81)RSIの直近22日間の回帰係数',
  '買残日数': '(98)信用買い残日数',
  '不安定率5':'(99)直近5日間の値幅不安定率',
  'タイプ分類':'(97)タイプ分類',
  '連日':     '(84)連続日数'
};
...
const FORMAT_DECIMAL_2 = [
  ...
  '週RSI','(130)ストキャスティクス%K','(132)週足ストキャスティクス%K','BB'
];
  ...
  try {
    for (let i = 0; i < numRows; i++) {
      ...
      const rowData = OUTPUT_HEADERS.map((label) => {
        ...
        const sourceLabel = SOURCE_HEADER_MAP[label] || label;
        const v1 = rec1 ? pickByHeader_(rec1, sourceLabel) : '';
        const v2 = rec2 ? pickByHeader_(rec2, sourceLabel) : '';
    ...
    // 書式（列位置は startCol 基準）
    const rightAlignIdx = ['終値','前日比','騰落率','出来高','時価総額','PER','PBR','DY','(86)10日間上昇率','(90)10日間下落率']
